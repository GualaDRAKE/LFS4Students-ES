...
|LSB Core:|install_initd, libcrypt.so.1 (puede proporcionarse con instrucciones opcionales para el paquete LFS Libxcrypt),<br> libncurses.so.5 (puede proporcionarse con instrucciones opcionales para el paquete LFS Ncurses),<br> libncursesw.so.5 (pero libncursesw.so.6 lo proporciona el paquete LFS Ncurses)|
|:------------:|:----------------------------------------------------------:|
|LSB Desktop:|libgdk-x11-2.0.so (pero libgdk-3.so lo proporciona el paquete BLFS GTK+-3), libgtk-x11-2.0.so<br> (pero libgtk- 3.so y libgtk-4.so lo proporcionan los paquetes BLFS GTK+-3 y GTK-4), libpng12.so<br> (pero libpng16.so lo proporciona el paquete BLFS Libpng). libQt\*.so.4 (pero libQt6\*.so.6 lo proporciona el paquete Qt6 de BLFS), libtiff.so.4 (pero libtiff.so.6 lo proporciona el paquete Libtiff de BLFS)|
...
• \[bdubbs\] - Actualización a Binutils-2.44.0. Corrige el problema
#5634.
...
• \[bdubbs\] - Actualización a iana-etc-20241220. Corrige el problema
#5006.
...
• \[bdubbs\] - Actualización a setuptools-75.6.0 (módulo de Python).
...
• \[bdubbs\] - Actualización a Python3-3.13.0. Corrige el problema n.°
5575.
...
de reglas generados desde *****/run/udev***** a
*****/etc/udev/rules*****. d si es necesario.
...
**Tiempo de compilación aproximado**: 0,4 - 32 SBU (normalmente unas 2,5
SBU)

**Espacio en disco necesario**: 1,7 - 14 GB (normalmente unos 2,3 GB)
...
Habilitar los encabezados del kernel a través de
...
* ción de modo y la compatibilidad con la consola Framebuffer*

* Enable legacy fbdev support for your modesetting driver and
...
Esta es una lista de vulnerabilidades de seguridad descubiertas en el
libro de LFS después de su publicación.

• Lista de Correo de Seguridad de Código Abierto

Esta es una lista de correo para debatir sobre fallos de seguridad,
conceptos y prácticas en la comunidad de código abierto.
